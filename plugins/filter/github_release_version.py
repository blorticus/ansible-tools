...
def match_lte(sorted_input_versions, match_key):
    ...
    if key_match_groups[2] is not None:
        ...
        while (next_element_after_match < len(sorted_input_versions) and
               sorted_input_versions[next_element_after_match][1] == wanted_major_version and
               sorted_input_versions[next_element_after_match][2] < wanted_minor_version):
            next_element_after_match += 1

        if key_match_groups[4] is not None:
            wanted_point_version = int(key_match_groups[4])
            while (next_element_after_match < len(sorted_input_versions) and
                   sorted_input_versions[next_element_after_match][2] == wanted_minor_version and
                   sorted_input_versions[next_element_after_match][3] <= wanted_point_version):
                next_element_after_match += 1
        else:
...
class TestModule(unittest.TestCase):
    ...
    def test_gte(self):
        ...
        self.assertEqual(
                github_release_version(TestModule.data_set_01, "gte", "3"),
                [],
